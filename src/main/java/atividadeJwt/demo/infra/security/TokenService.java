    ...
    public String geradorToken (Usuario usuario){
        try{
            ...
            String token = JWT.create()
                    .withIssuer("atividadeJwt") // possivel erro pode ocorrer aqui "nome da aplicacao"
                    .withSubject(usuario.getEmail())
                    .withExpiresAt(this.geradorExpiracao())
                    .sign(algorithm);
            return token;
        }catch (JWTCreationException ex){
            throw new RuntimeException("Erro enquanto estava autentiacando");
        }
    }

    public String validarToiken(String token){
        try{
            Algorithm algorithm = Algorithm.HMAC256(secreto);
